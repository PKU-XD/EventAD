...
def tracks_to_array(tracks):
    return np.stack([tracks['x'], tracks['y'], tracks['w'], tracks['h'], tracks['class_id'],tracks['track_id']], axis=1)
...
class DSEC(Dataset):
    ...
    # MAPPING = dict(pedestrian="anomaly", rider=None, car="normal", bus="car", truck="car", bicycle=None,
    #                motorcycle=None, train=None)
    def __init__(self,
                 args,
                 root: Path,
        ...
        split_config = None
        if not demo:
            split_config = yaml_file_to_dict(Path(args.split))
        ...
        self.height = self.dataset.height // scale
    ...
    def __getitem__(self, idx):
        dataset, image_index_pairs, track_masks, idx = self.rel_index(idx) # image_index_pairs :[[0,1],[1,2]]
        image_index_0, image_index_1 = image_index_pairs[idx] # 0,1
        image_ts_0, image_ts_1 = dataset.images.timestamps[[image_index_0, image_index_1]]# 55298357580,55298407585
        # [(55298357580, 453.0, 205.0, 24.0, 55.0, 0, 0.9994024, 186), (55298357580, 251.0, 214.0, 44.0, 36.0, 2, 0.9999051, 175), (55298357580, 170.0, 205.0, 37.0, 36.0, 3, 0.99535215, 193)]
        detections_0 = self.dataset.get_tracks(image_index_0, mask=track_masks, directory_name=dataset.root.name)
        # [(55298407585, 457.0, 202.0, 25.0, 57.0, 0, 0.9991948, 186), (55298407585, 252.0, 213.0, 44.0, 36.0, 2, 0.99993336, 175), (55298407585, 170.0, 202.0, 40.0, 36.0, 3, 0.9821702, 193)]
        ...
        # Data(x=[58625, 1], y=[58625], pos=[58625, 2], p=[58625, 1], t=[58625], bbox=[3, 5], bbox0=[3, 5], t0=55298357580, t1=55298407585, width=320, height=215, time_window=1000000, image=[1, 3, 215, 320], sequence='zurich_city_14_b')
        data = to_data(**events, bbox=tracks_to_array(detections_1), bbox0=tracks_to_array(detections_0), t0=image_ts_0, t1=image_ts_1,
                       track_ids=detections_1['track_id'], track_ids0=detections_0['track_id'],
