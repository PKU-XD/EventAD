...
class DAGR(YOLOX):
    ...
    def forward(self, x: Data, reset=True, return_targets=True, filtering=True):
        ...
        if self.training:
            targets = convert_to_training_format(x.bbox, x.batch, self.batch_size)

            if self.backbone.use_image:
                targets0 = convert_to_training_format(x.bbox0, x.batch, self.batch_size)
        ...
        x.reset = reset
        # GNN's output 
        outputs = YOLOX.forward(self, x) # torch.Size([batch_size, 175, 7])
        # batch_size -> frame -> multi bbox scores labels
        # {'boxes': tensor([[ -1.0758, 100.2616,  39.7441, 207.3679],
        #         [101.6575,  98.0414, 118.7784, 117.9262]], device='cuda:0'), 
        # 'scores': tensor([0.6764, 0.0026], device='cuda:0'), 
        ...
        ret = [detections]
        # {'boxes': tensor([[  0.0000, 105.0000,  46.5000, 184.0000]], device='cuda:0'), 
    ...
    def extract_features(self, x):
        ...
        with torch.no_grad():
            ...
            # 如果features是列表或元组（不同尺度的特征），可以选择其中一个或合并它们
