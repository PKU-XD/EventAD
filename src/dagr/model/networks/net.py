...
class Net(torch.nn.Module):
    ...
    def forward(self, data: Data, reset=True):
        ...
        # Event feature
        output = [out3, out4]
        # out4 是经过第四层池化操作后得到的图数据结构，与 out3 类似，但节点和边的数量不同：
        # x=[278, 64]: 图节点的特征矩阵，278 表示节点数量，64 表示每个节点的特征维度。
        # edge_index=[2, 1233]: 边的连接索引，1233 表示图中共有 1233 条边。
        # pos=[278, 3]: 每个节点的空间位置，278 表示节点数量，3 表示每个节点的空间位置。
        # batch=[278]: 批次索引，指示节点属于哪个 batch。
        # height=[8] 和 width=[8]: 图的高度和宽度，表示经过池化后的图像特征空间维度。
        # edge_attr=[1233, 3]: 边的属性矩阵，每条边有 3 维的特征。
        # skipped=True: 标识是否进行了特征跳跃连接，表示将图像特征与图节点特征进行了融合。
        # num_image_channels=64: 图像特征通道数，表示与图像特征结合后，每个节点的特征维度增加到了 64。
        # pooling=[3]: 表示池化操作后的空间维度，这里是 3，表明进行了空间降采样。
        if self.use_image:
